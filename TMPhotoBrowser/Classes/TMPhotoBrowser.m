...
@property (nonatomic,strong) TMPhotoBrowserView *photoBrowserView;
@property (nonatomic,assign) UIDeviceOrientation orientation;
@property (nonatomic,assign) TMPhotoBrowserStyle photoBrowserStyle;
@end
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        self.backgroundColor = TMPhotoBrowserBackgrounColor;
...
//    NSLog(@"layoutSubviews -- ");
    ...
    rect.size.width += TMPhotoBrowserImageViewMargin * 2;
    ...
    CGFloat y = 0;
    __block CGFloat w = _scrollView.frame.size.width - TMPhotoBrowserImageViewMargin * 2;
    CGFloat h = _scrollView.frame.size.height;
    [_scrollView.subviews enumerateObjectsUsingBlock:^(TMPhotoBrowserView *obj, NSUInteger idx, BOOL *stop) {
        CGFloat x = TMPhotoBrowserImageViewMargin + idx * (TMPhotoBrowserImageViewMargin * 2 + w);
...
-(void)setSourceImagesContainerView:(UIView *)sourceImagesContainerView{
    ...
    _imageArray = nil;
    _photoBrowserStyle = TMPhotoBrowserStyleDefault;
}
...
- (void)setImageArray:(NSArray *)imageArray{
    ...
    _photoBrowserStyle = TMPhotoBrowserStyleSimple;
}
...
- (UIImageView *)tempView{
    if (!_tempView) {
        TMPhotoBrowserView *photoBrowserView = _scrollView.subviews[self.currentImageIndex];
        ...
        //这边的contentmode要跟 TMPhotoGrop里面的按钮的 contentmode保持一致（防止最后出现闪动的动画）
...
- (UIView *)coverView{
    if (!_coverView) {
        ...
        _coverView.backgroundColor = TMPhotoBrowserBackgrounColor;
    }
    return _coverView;
}

- (void)setPhotoBrowserView:(TMPhotoBrowserView *)photoBrowserView{
...
- (void)setupScrollView
{
    ...
    for (int i = 0; i < self.imageCount; i++) {
        TMPhotoBrowserView *view = [[TMPhotoBrowserView alloc] init];
...
{
    TMPhotoBrowserView *view = _scrollView.subviews[index];
...
-(void)onDeviceOrientationChange
{
    ...
    TMPhotoBrowserView *currentView = _scrollView.subviews[self.currentImageIndex];
    UIDeviceOrientation orientation = [UIDevice currentDevice].orientation;
...
- (void)showFirstImage
{
    ...
    @weakify(self);
    if (_photoBrowserStyle == TMPhotoBrowserStyleDefault) {
...
- (void)hideAnimation{
    ...
    [UIView animateWithDuration:TMPhotoBrowserHideImageAnimationDuration animations:^{
        @strongify(self);
        if (self->_photoBrowserStyle == TMPhotoBrowserStyleDefault) {
...
- (void)bounceToOrigin{
    ...
    [UIView animateWithDuration:TMPhotoBrowserHideImageAnimationDuration animations:^{
        ...
        self.backgroundColor = TMPhotoBrowserBackgrounColor;
        self->_contentView.backgroundColor = TMPhotoBrowserBackgrounColor;
        UIView *view = [self getSourceView];
        view.hidden = NO;
    }];
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    for (TMPhotoBrowserView *view in _scrollView.subviews) {
...
{
    TMPhotoBrowserView *view = _scrollView.subviews[self.currentImageIndex];
...
- (void)show
{
    ...
    _contentView.backgroundColor = TMPhotoBrowserBackgrounColor;
    ...
    [self performSelector:@selector(onDeviceOrientationChangeWithObserver) withObject:nil afterDelay:TMPhotoBrowserShowImageAnimationDuration + 0.2];
}
